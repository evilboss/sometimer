...
export default {
  ...
  exportSummary({Meteor, LocalState}, teamId, from, to, name, summaryList){
    var nameFile = `${(name) ? name : teamId}-summary-${from.toString()}-${to.toString()}.csv`;
    Meteor.call('download.team.summary.csv', teamId, from, to, summaryList, function (err, fileContent) {
      if (fileContent) {
        Bert.alert({
          type: 'success',
          style: 'growl-bottom-right',
          title: 'Summary Export',
          message: 'Exporting Summary Please wait',
          icon: 'fa-table'
        });
        var blob = new Blob([fileContent], {type: "text/plain;charset=utf-8"});
        saveAs(blob, nameFile);
